...
class TableReservations(db.Model):
    ...
    order_id = db.Column(db.String(50), db.ForeignKey('orders.order_id'))  # Link to order

    def __repr__(self):
        return f"<TableReservations(table_id={self.table_id}, guest_count={self.guest_count}, status={self.status}, order_id={self.order_id})>"
